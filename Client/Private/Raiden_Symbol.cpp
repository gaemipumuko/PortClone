...
CRaiden_Symbol::CRaiden_Symbol(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CEffect(pDevice, pContext)
{
}

CRaiden_Symbol::CRaiden_Symbol(const CRaiden_Symbol& rhs)
	: CEffect(rhs)
{
}

HRESULT CRaiden_Symbol::Add_Component(void* pArg)
{
	...
	/* For.Com_Texture */
	if (FAILED(CGameObject::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Texture_Raiden_Symbol"),
...
HRESULT CRaiden_Symbol::Bind_ShaderResources()
{
...
HRESULT CRaiden_Symbol::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CRaiden_Symbol::Initialize_Clone(void* pArg)
{
...
HRESULT CRaiden_Symbol::PreviousTick(_float fTimeDelta)
{
	...
	if (m_fLifeTime <= 0.5f)
	{
		...
		vScale.m128_f32[1] = vScale.m128_f32[1] - fTimeDelta * 2.5f;
		if (vScale.m128_f32[1] < 0.f)
			vScale.m128_f32[1] = 0.f;
...
HRESULT CRaiden_Symbol::Tick(_float fTimeDelta)
{
	if (m_fLifeTime <= 0.f)
		Set_Destroy(TRUE);
	...
	//if (vSelfScale.m128_f32[0] >= 3.f || m_bScaleSmall)
	//{
	//	m_fColorRate += fTimeDelta;
	//	m_bScaleSmall = TRUE;
	//}
	//else
	//{
	//	vSelfScale.m128_f32[0] += fTimeDelta * 1.5f;
	//	vSelfScale.m128_f32[1] += fTimeDelta * 1.5f;
	//}
...
HRESULT CRaiden_Symbol::LateTick(_float fTimeDelta)
{
...
HRESULT CRaiden_Symbol::Render()
{
	...
	if (FAILED(m_pShaderCom->Begin(VTXTEX_RAIDEN_SYMBOL)))
		return E_FAIL;
...
CRaiden_Symbol* CRaiden_Symbol::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CRaiden_Symbol* pInstance = new CRaiden_Symbol(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Create : CRaiden_Symbol"));
...
CGameObject* CRaiden_Symbol::Clone(void* pArg)
{
	CRaiden_Symbol* pInstance = new CRaiden_Symbol(*this);

	if (FAILED(pInstance->Initialize_Clone(pArg)))
	{
		MSG_BOX(TEXT("Failed to Clone : CRaiden_Symbol"));
		Safe_Release(pInstance);
	}
