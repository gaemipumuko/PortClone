...
CGlider_Origin::CGlider_Origin(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
    : CPart_Glider(pDevice, pContext)
{
}

CGlider_Origin::CGlider_Origin(const CGlider_Origin& rhs)
    : CPart_Glider(rhs)
{
}

HRESULT CGlider_Origin::Invalidate_Animation(_float fTimeDelta)
{
	...
	if (eState == FSM_FLY_START)
	{
		...
		m_pModelCom->Play_Animation(FALSE, m_pTransformCom, fTimeDelta * 1.5f);
	}
	else
	{
...
HRESULT CGlider_Origin::Initialize_Prototype()
{
...
HRESULT CGlider_Origin::Initialize_Clone(void* pArg)
{
...
HRESULT CGlider_Origin::PreviousTick(_float fTimeDelta)
{
...
HRESULT CGlider_Origin::Tick(_float fTimeDelta)
{
...
HRESULT CGlider_Origin::LateTick(_float fTimeDelta)
{
...
CGlider_Origin* CGlider_Origin::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CGlider_Origin* pInstance = new CGlider_Origin(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Create : CGlider_Origin"));
...
CGameObject* CGlider_Origin::Clone(void* pArg)
{
	CGlider_Origin* pInstance = new CGlider_Origin(*this);

	if (FAILED(pInstance->Initialize_Clone(pArg)))
	{
		MSG_BOX(TEXT("Failed to Clone : CGlider_Origin"));
		Safe_Release(pInstance);
	}
