...
CBody_Slime_Ice::CBody_Slime_Ice(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CPart_Body(pDevice, pContext)
{
}

CBody_Slime_Ice::CBody_Slime_Ice(const CBody_Slime_Ice& rhs)
	: CPart_Body(rhs)
{
}

HRESULT CBody_Slime_Ice::Invalidate_Animation(_float fTimeDelta)
{
	...
	case FSM_SLIME_IDLE:
		...
		if (fDuration >= m_pModelCom->Get_AnimationDuration(14) * 0.48f)
		{
	...
	case FSM_SLIME_WALK:
		...
		{
			m_pModelCom->Set_AnimationCurrentDuration(m_pModelCom->Get_AnimationDuration(4) * 0.f);
		}
	break;
	...
	case FSM_SLIME_ATTACK:
	{
		...
		if (fDuration >= m_pModelCom->Get_AnimationDuration(11) * 0.48f)
		{
			if (m_bSoundPlayed == false)
			{
	...
	case FSM_SLIME_BREATHATTACK:
	{
		m_vPrevRootTranslation = m_vRootTranslation;
		m_pModelCom->Set_Animation(2);
		fRate = m_pModelCom->Play_RootAnimation(FALSE, m_pParentTransformCom, fTime * 0.8f, m_vRootTranslation);
		_vector vCurrentTranslation = m_vPrevRootTranslation - m_vRootTranslation;
		Set_ControllerTranslation(vCurrentTranslation);
		...
		if (fDuration >= m_pModelCom->Get_AnimationDuration(2) * 0.1f)
		{
			if (m_bSoundPlayed == false)
			{
				m_pGameInstance->PlaySoundW(L"Slime_Breath.mp3");
				m_bSoundPlayed = true;
			}
		}
	...
	case FSM_SLIME_HIT_THROW_START:
	{
		...
		Set_ControllerTranslation(vCurrentTranslation);
		if (fRate >= (m_pModelCom->Get_AnimationDuration(9) * 0.6f))
		{
	...
	case FSM_SLIME_HIT_THROW:
	{
		...
		vCurrentTranslation.m128_f32[1] = 0.f;
		Set_ControllerTranslation(vCurrentTranslation);
		if (fRate >= (m_pModelCom->Get_AnimationDuration(7) * 0.6f))
		{
	...
	case FSM_SLIME_DIE1:
		...
		if (fRate >= (m_pModelCom->Get_AnimationDuration(4)))
		{
			...
			if (fDuration >= m_pModelCom->Get_AnimationDuration(4) * 0.6f)
			{
				if (m_bSoundPlayed == false)
				{
			...
			{
				m_fDissolve += 0.005f;
				m_vDissolveColor = _float3(0.f, 1.f, 1.f);
			}
			else
				m_bDead = TRUE;
		}
...
HRESULT CBody_Slime_Ice::Add_Component(void* pArg)
{
...
HRESULT CBody_Slime_Ice::Initialize_Prototype()
{
...
HRESULT CBody_Slime_Ice::Initialize_Clone(void* pArg)
{
...
HRESULT CBody_Slime_Ice::PreviousTick(_float fTimeDelta)
{
	if (FAILED(__super::PreviousTick(fTimeDelta)))
		return E_FAIL;

	if (isHostElement(ELEMENT_FROST) == FALSE)
	{
		_bool bResetElement = FALSE;
		if (m_eReceiveElement == ELEMENT_END)
			bResetElement = TRUE;
		CPart_Body::Set_HostElement(ELEMENT_FROST, TRUE);
...
HRESULT CBody_Slime_Ice::Tick(_float fTimeDelta)
{
...
HRESULT CBody_Slime_Ice::LateTick(_float fTimeDelta)
{
...
HRESULT CBody_Slime_Ice::Render()
{
...
void CBody_Slime_Ice::Release_ColliderID()
{
	m_pGameInstance->Release_Collider_Object(COLL_MONSTER_BODY, m_iColliderID);
}

CBody_Slime_Ice* CBody_Slime_Ice::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CBody_Slime_Ice* pInstance = new CBody_Slime_Ice(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Create : CBody_Slime_Ice"));
...
CGameObject* CBody_Slime_Ice::Clone(void* pArg)
{
	CBody_Slime_Ice* pInstance = new CBody_Slime_Ice(*this);

	if (FAILED(pInstance->Initialize_Clone(pArg)))
	{
		MSG_BOX(TEXT("Failed to Clone : CBody_Slime_Ice"));
		Safe_Release(pInstance);
	}
