...
CBody_TelePort::CBody_TelePort(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CPart_Body(pDevice, pContext)
{
}

CBody_TelePort::CBody_TelePort(const CBody_TelePort& rhs)
	: CPart_Body(rhs)
{
}

HRESULT CBody_TelePort::Invalidate_Animation(_float fTimeDelta)
{
	FSM_TELEPORT_STATE eState = static_cast<FSM_TELEPORT_STATE>(m_pParentFSMCom->Get_CurrentState());
	
	_float fRate = 0.f;

	switch (eState)
	{
	case FSM_TELEPORT_IDLE:
		m_pModelCom->Set_Animation(2);
		m_pModelCom->Play_PlayerRootAnimation(TRUE, m_pParentTransformCom, fTimeDelta, m_vRootTranslation);
	break;

	case FSM_TELEPORT_CLOSE:
		m_pModelCom->Set_Animation(1);
		m_pModelCom->Play_PlayerRootAnimation(TRUE, m_pParentTransformCom, fTimeDelta, m_vRootTranslation);
	break;

	case FSM_TELEPORT_OPEN:
	{
		...
		{
			m_pParentFSMCom->Change_ForceState(FSM_TELEPORT_IDLE);
		}
	}
	break;
	}
...
HRESULT CBody_TelePort::Add_Component(void* pArg)
{
...
HRESULT CBody_TelePort::Initialize_Prototype()
{
...
HRESULT CBody_TelePort::Initialize_Clone(void* pArg)
{
...
HRESULT CBody_TelePort::PreviousTick(_float fTimeDelta)
{
...
HRESULT CBody_TelePort::Tick(_float fTimeDelta)
{
...
HRESULT CBody_TelePort::LateTick(_float fTimeDelta)
{
...
void CBody_TelePort::Release_ColliderID()
{
...
CBody_TelePort* CBody_TelePort::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CBody_TelePort* pInstance = new CBody_TelePort(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Create : CBody_TelePort"));
...
CGameObject* CBody_TelePort::Clone(void* pArg)
{
	CBody_TelePort* pInstance = new CBody_TelePort(*this);

	if (FAILED(pInstance->Initialize_Clone(pArg)))
	{
		MSG_BOX(TEXT("Failed to Clone : CBody_TelePort"));
		Safe_Release(pInstance);
	}
