...
CStaff_Water::CStaff_Water(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
    : CPart_Weapon(pDevice, pContext)
{
}

CStaff_Water::CStaff_Water(const CStaff_Water& rhs)
    : CPart_Weapon(rhs)
{
}

_matrix CStaff_Water::Get_CombinedMatrix()
{
	return _matrix();
}

HRESULT CStaff_Water::Invalidate_Animation(_float fTimeDelta)
{
	return S_OK;
}

void CStaff_Water::Release_ColliderID()
{
	m_pGameInstance->Release_Collider_Object(COLL_MONSTER_WEAPON, m_iColliderID);
}

HRESULT CStaff_Water::Initialize_Prototype()
{
...
HRESULT CStaff_Water::Initialize_Clone(void* pArg)
{
...
HRESULT CStaff_Water::PreviousTick(_float fTimeDelta)
{
...
HRESULT CStaff_Water::Tick(_float fTimeDelta)
{
...
HRESULT CStaff_Water::LateTick(_float fTimeDelta)
{
...
CStaff_Water* CStaff_Water::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CStaff_Water* pInstance = new CStaff_Water(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Create : CStaff_Water"));
...
CGameObject* CStaff_Water::Clone(void* pArg)
{
	CStaff_Water* pInstance = new CStaff_Water(*this);

	if (FAILED(pInstance->Initialize_Clone(pArg)))
	{
		MSG_BOX(TEXT("Failed to Clone : CStaff_Water"));
		Safe_Release(pInstance);
	}
