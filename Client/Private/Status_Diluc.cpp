...
CStatus_Diluc::CStatus_Diluc(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CPart_Status(pDevice, pContext)
{
}

CStatus_Diluc::CStatus_Diluc(const CStatus_Diluc& rhs)
	: CPart_Status(rhs)
{
}

HRESULT CStatus_Diluc::Set_Status()
{
	/* 현재, 최대, 증가율, 돌파증가율 */
	...
	m_EntityStatus.fHp = { 12981.f, 12981.f, 134.f, 10.f };
	m_EntityStatus.fAtk = { 335.f, 0.f, 4.f, 10.f };
	...
	m_EntityStatus.fReuseTime = { 0.f, 0.f, 10.f, 12.f }; /* 원소스킬 대기시간, 원소폭발 대기시간 */
...
void CStatus_Diluc::Level_BreakThrough()
{
}

HRESULT CStatus_Diluc::Initialize_Prototype()
{
...
HRESULT CStatus_Diluc::Initialize_Clone(void* pArg)
{
...
HRESULT CStatus_Diluc::PreviousTick(_float fTimeDelta)
{
	return S_OK;
}

HRESULT CStatus_Diluc::Tick(_float fTimeDelta)
{
	return S_OK;
}

HRESULT CStatus_Diluc::LateTick(_float fTimeDelta)
{
	return S_OK;
}

CStatus_Diluc* CStatus_Diluc::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CStatus_Diluc* pInstance = new CStatus_Diluc(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Create : CStatus_Diluc"));
...
CGameObject* CStatus_Diluc::Clone(void* pArg)
{
	CStatus_Diluc* pInstance = new CStatus_Diluc(*this);

	if (FAILED(pInstance->Initialize_Clone(pArg)))
	{
		MSG_BOX(TEXT("Failed to Clone : CStatus_Diluc"));
		Safe_Release(pInstance);
	}
